...
export function Balance() {
  ...
  function getSubTitle(pkg: Balance) {
    ...
    return (
      ...
      (pkg.tokens
        ? `<li>📝${prefix} <span style="font-size: 17px;">${pkg.tokens === -1 ? "无限" : pkg.tokens
        }</span> tokens</li>`
        : "") +
      (pkg.chatCount
        ? `<li>🚘${prefix} <span style="font-size: 17px;">${pkg.chatCount === -1 ? "无限" : pkg.chatCount
        }</span> 次基础聊天（GPT3.5）</li>`
        : "") +
      (pkg.advancedChatCount
        ? `<li>🚀${prefix} <span style="font-size: 17px;">${pkg.advancedChatCount === -1 ? "无限" : pkg.advancedChatCount
        }</span> 次高级聊天（GPT4）</li>`
        : "") +
      (pkg.drawCount
        ? `<li>🎨${prefix} <span style="font-size: 17px;">${pkg.drawCount === -1 ? "无限" : pkg.drawCount
        }</span> 次AI绘画</li>`
        : "") +
      `<li>⏱️到期时间：${pkg.expireTime}</span></li>` +
      (isMobileScreen
        ? `<li>🛍️${(pkg.sourceId === 6 ? "兑换" : pkg.sourceId === 1 ? "注册赠送" : pkg.sourceId === 5 ? "系统赠送" : "购买")+
        `时间：${pkg.createTime}`}</li>`
        : "") +
      `<li>${pkg.expired ? "🔴套餐过期" : "🟢套餐有效"}</li>` +
      `</ul>`
    );
  }
                    ...
                    {!isMobileScreen && (
                      <div style={{ margin: "10px 0" }}>
                        <div style={{ fontSize: "10px" }}>
                          {`🛍️${pkg.sourceId === 6 ? "兑换" : pkg.sourceId === 1 ? "注册赠送" : pkg.sourceId === 5 ? "系统赠送" : "购买"}时间：${pkg.createTime}`}
                        </div>
                      </div>
                    )}
