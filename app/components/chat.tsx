...
import { IMAGE_HELLO } from "../constant";
...
export function ChatActions(props: {
  ...
  const currentContentType = chatStore.currentSession().mask.modelConfig.contentType;
  // 使用useMemo来记忆化models数组
  const models = useMemo(() => {
    // 从availableModels中提取模型名称和contentType
    const modelNamesWithContentType = availableModels.map(model => {
      return {
        name: model.name,
    ...
    if (modelNamesWithContentType.length > 0) {
      return modelNamesWithContentType;
    } else {
      return [{ name: "No Models Available", contentType: "Unknown" }];
    }
  }, [availableModels]);
  ...
  return (
    <div className={styles["chat-input-actions"]}>
      ...
      <ChatAction
        ...
        text={currentModel}
        icon={props.contentType === "Image" ? <ImageIcon /> : <RobotIcon />}
      />
      ...
      {showModelSelector && (
        <Selector
          ...
          onSelection={(s) => {
            ...
            // 查找所选模型的contentType
            const selectedModel = models.find(m => m.name === s[0]);
            const selectedContentType = selectedModel ? selectedModel.contentType : undefined;

            chatStore.updateCurrentSession((session) => {
              session.mask.modelConfig.model = s[0] as ModelType;

              // 检查并分配selectedContentType
              if (selectedContentType === "Text" || selectedContentType === "Image") {
                session.mask.modelConfig.contentType = selectedContentType;
              }
...
function _Chat() {
  ...
  const [pluginModels, setPluginModels] = useState<PluginActionModel[]>([]);
  useEffect(() => {
  ...
  if (
    ...
    if (!authStore.token) {
      ...
      // 添加判断，检查 session.mask.modelConfig.model 是否包含 "Midj"
  if (session.mask.modelConfig.model && session.mask.modelConfig.model.includes("Midj")) {
    copiedHello.content = IMAGE_HELLO;
  }
          ...
          <div className="window-header-sub-title">
            {
              Locale.Chat.SubTitle(
                ...
                (!session.mask.name || session.mask.name === Locale.Home.NewChat) ? 'Aivesa' : session.mask.name
              )
            }
          </div>
                            ...
                            {[1, 2, 3, 4].map((index) => {
                              return (
                                <button
                                  ...
                                  title={`放大第${index}张图`}  // 添加 title 属性
                                >
                                  U{index}
                                </button>
                              );
                              ...
                              {[1, 2, 3, 4].map((index) => {
                                return (
                                  <button
                                    ...
                                    className={`${styles["chat-message-mj-action-btn"]} clickable`}
                                    title={`转换第${index}张图`}  // 添加 title 属性
                                  >
                                    V{index}
                                  </button>
                                );
                      ...
                      message.attr?.status === "SUCCESS" && (
                        ...
                        >
                          {!message.attr?.direction && (
                            <div style={{ display: "flex", alignItems: "center" }}>
                              ...
                              {["Strong", "Subtle"].map((strength) => {
                                let buttonText = strength === "Strong" ? "主体" : "细节";
                                return (
                                  <button
                                    key={strength}
                                    onClick={() =>
                                      doSubmit(
                                        `VARY::${strength.toLocaleUpperCase()}::${message.attr.taskId
                                        }`,
                                      )
                                    }
                                    className={`${styles["chat-message-mj-action-btn"]} clickable ${styles["vary"]}`}
                                    title={buttonText}  // 添加 title 属性
                                  >
                                    Vary ({buttonText})
                                  </button>
                                );
                          ...
                          <div style={{ display: "flex", alignItems: "center" }}>
                            <span>边缘填充：</span>
                            {[1.5, 2].map((index) => {
                              let buttonText = index === 1.5 ? "1.5 倍" : "2 倍";
                              return (
                                <button
                                  key={index}
                                  onClick={() =>
